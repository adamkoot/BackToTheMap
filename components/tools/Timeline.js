...
export default function Timelinee({ route }) {
  ...
  function renderDetail(rowData, sectionID, rowID) {
    ...
    return (
      <>
        ...
        >
          <Text style={{ color: "white" }}>{i18n.t("more")}</Text>
        </TouchableOpacity>
      </>
    );
  }

  const [cityInfo, setCityInfo] = useState("Kraków");
  useEffect(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.list}>
        <Timeline
          circleColor="#00305B"
          data={links[i18n.locale.slice(0, 2)][cityInfo]}
          innerCircle={"dot"}
          timeStyle={{
            textAlign: "center",
